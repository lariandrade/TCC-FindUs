...
public class DenunciaController {
    ...
    @GetMapping("/denunciarPrestador")
    public String denunciarPrestador(@RequestParam("idprestador") Long idPrestador, @RequestParam("idcliente") Long idCliente, Model model) {

        Prestador usuario = prestadorService.findById(idPrestador);
        Cliente denunciado = clienteService.findById(idCliente);

        model.addAttribute("usuario", usuario);
        model.addAttribute("denunciado", denunciado);

        return "geral/denunciarPrestador";
    ...
    @PostMapping("/enviar-denuncia-prestador")
    public String enviarDenunciaPrestador(@RequestParam("corpo") String motivoDenuncia,
                                        @RequestParam("idUsuario") Long idUsuario,
                                        @RequestParam("idDenunciado") Long idDenunciado) throws MessagingException {
        ...
        Cliente cliente = clienteService.findById(idDenunciado);
        ...
        String mensagemFormatada = "<html><head>"
                ...
                + "</style>"
                + "</head><body>"
                + "<h3>Informações do cliente denunciado:</h3>"
                + "<br>"
                + "<p>Nome completo: " + cliente.getUserNome() + "</p>"
                + "<p>CPF/CNPJ: " + cliente.getUserCPF_CNPJ() + "</p>"
                + "<p>Email: " + cliente.getUserEmail() + "</p>"
                + "<br>"
                + "<h3>Reclamação reportada:</h3>"
                + "<p>"+motivoDenuncia+"</p>"
                + "</body></html>";
        ...
        return "redirect:/visualizarCliente?idCliente=" + idDenunciado + "&idPrestador=" + idUsuario;
